...
ShaderPrograms::ShaderPrograms(const ShaderConfig &config,
                               const Renderer &renderer, const Window &window,
                               const filesys::path &shader_folder) {
#define lambda [&](const int p, const Buffer &b)
  builtin_uniforms = {{"vec2", "iMouse",
                       lambda{glUniform2f(get_uniform_loc(p, 0), window.mouse.x,
                                          window.mouse.y);
}
}
, {"bool", "iMouseDown",
   lambda{glUniform1i(get_uniform_loc(p, 1), window.mouse.down);
}
}
, {"vec2", "iMouseDownPos",
   lambda{glUniform2f(get_uniform_loc(p, 2), window.mouse.last_down_x,
                      window.mouse.last_down_y);
}
}
, {"vec2", "iRes",
   lambda{glUniform2f(get_uniform_loc(p, 3), window.width, window.height);
}
}
, {"float", "iTime",
   lambda{glUniform1f(get_uniform_loc(p, 4), renderer.elapsed_time);
}
}
, {"int", "iFrame",
   lambda{glUniform1i(get_uniform_loc(p, 5), renderer.frame_counter);
}
}
, {"float", "iNumGeomIters",
   lambda{glUniform1f(get_uniform_loc(p, 6), float(b.geom_iters));
}
}
, {"sampler1D", "iSoundR", lambda{glUniform1i(get_uniform_loc(p, 7), 0);
}
}
, // texture_unit 0
    {"sampler1D", "iSoundL", lambda{glUniform1i(get_uniform_loc(p, 8), 1);
}
}
, // texture_unit 1
    {"sampler1D", "iFreqR", lambda{glUniform1i(get_uniform_loc(p, 9), 2);
}
}
, // texture_unit 2
    {"sampler1D", "iFreqL", lambda{glUniform1i(get_uniform_loc(p, 10), 3);
}
}
, // texture_unit 3
{
  "vec2", "iBuffRes", lambda {
    glUniform2f(get_uniform_loc(p, 11), float(b.width), float(b.height));
  }
}
}
;
...
for (const Buffer &b : config.mBuffers)
  compile_buffer_shaders(shader_folder, b.name, uniform_header.str(),
                         b.uses_default_geometry_shader);
compile_buffer_shaders(shader_folder, config.mImage.name, uniform_header.str(),
                       config.mImage.uses_default_geometry_shader);
...
GLint ShaderPrograms::get_uniform_loc(int program_i, int uniform_i) const {
  if (program_i >= mPrograms.size()) {
    cout << "program_i = " + to_string(program_i) + " is not a program index"
         << endl;
    return 0;
  }
  if (uniform_i >= mUniformLocs[program_i].size()) {
    cout << "uniform_i = " + to_string(uniform_i) + " is not a uniform index"
         << endl;
    return 0;
  }
...
void ShaderPrograms::compile_buffer_shaders(
    const filesys::path &shader_folder, const string &buff_name,
    const string &uniform_header, const bool uses_default_geometry_shader) {
  ...
  if (frag_str.str().find("mainImage",
                          uniform_header.size() + version_header.size()) !=
      std::string::npos)
    frag_str
