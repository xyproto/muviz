...
// TODO Test the output of the shaders. Use dummy data in AudioData. Compute
// similarity between expected images and produced images.

// TODO add a previously rendered uniform so that a single buffer can be
// repetitvely applied

// TODO buffer.size option is ShaderConfig is not rendered correctly, rendering
// to half res and then upscaling in image.frag doesn't work as expected
...
Renderer::Renderer(const ShaderConfig &config, const Window &window)
    : config(config), window(window), frame_counter(0), num_user_buffers(0),
  ...
  if (config.mBlend) {
    // I chose the following blending func because it allows the user to
    // completely replace the colors in the buffer by setting their output alpha
    // to 1. unfortunately it forces the user to make one of three choices: 1)
    // replace color in the framebuffer 2) leave framebuffer unchanged 3) mix
    // new color with old color
    glEnable(GL_BLEND);
  ...
  // glEnable(GL_DEPTH_TEST); // maybe allow as option so that geom shaders are
  // more useful
  ...
  for (int i = 0; i < num_user_buffers; ++i) {
    ...
    glTexImage2D(
        GL_TEXTURE_2D, // which binding point on the current active texture
        0,
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, buff.width, buff.height, 0,
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    glTexImage1D(GL_TEXTURE_1D, 0, GL_R32F, VISUALIZER_BUFSIZE, 0, GL_RED,
                 GL_FLOAT, nullptr);
...
void Renderer::update(AudioData &data) {
  ...
  // because I'm using four 1D textures I need to store them in separate texture
  // units
  //
  // glUniform1i(textureLoc, int) sets what texture unit the sampler in the
  // shader reads from
  //
  // A texture binding created with glBindTexture remains active until a
  // different texture is bound to the same target (in the active unit? I
  // think), or until the bound texture is deleted with glDeleteTextures. So I
  // do not need to rebind glBindTexture(GL_TEXTURE_1D, tex[X]);
  data.mtx.lock();
  glActiveTexture(GL_TEXTURE0 + 0);
  glTexSubImage1D(GL_TEXTURE_1D, 0, 0, VISUALIZER_BUFSIZE, GL_RED, GL_FLOAT,
                  data.audio_r);
...
void Renderer::render() {
  ...
  for (const int r : config.mRender_order) {
    ...
    glClearColor(buff.clear_color[0], buff.clear_color[1], buff.clear_color[2],
                 1.f);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    glDrawArrays(GL_POINTS, 0, buff.geom_iters);
    buffers_last_drawn[r] += 1;
    buffers_last_drawn[r] %= 2;
    // bind most recently drawn texture to texture unit r so other buffers can
    // use it
    glBindTexture(GL_TEXTURE_2D, fbo_textures[2 * r + buffers_last_drawn[r]]);
  }
...
void Renderer::upload_uniforms(const Buffer &buff, const int buff_index) const {
  ...
  for (int i = 0; i < config.mUniforms.size(); ++i) {
    ...
    switch (uv.size()) {
    case 1:
      glUniform1f(shaders->get_uniform_loc(buff_index, uniform_offset + i),
                  uv[0]);
      break;
    case 2:
      glUniform2f(shaders->get_uniform_loc(buff_index, uniform_offset + i),
                  uv[0], uv[1]);
      break;
    case 3:
      glUniform3f(shaders->get_uniform_loc(buff_index, uniform_offset + i),
                  uv[0], uv[1], uv[2]);
      break;
    case 4:
      glUniform4f(shaders->get_uniform_loc(buff_index, uniform_offset + i),
                  uv[0], uv[1], uv[2], uv[3]);
      break;
    }
  }
