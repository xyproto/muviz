...
static AudioOptions parse_audio_options(rj::Document &user_conf) {
  ...
  if (audio_options.HasMember("fft_smooth")) {
    ...
    if (!fft_smooth.IsNumber())
      throw runtime_error(
          "fft_smooth must be a number between in the interval [0, 1]");
  ...
  if (audio_options.HasMember("wave_smooth")) {
    ...
    if (!wave_smooth.IsNumber())
      throw runtime_error(
          "wave_smooth must be a number between in the interval [0, 1]");
...
static Buffer parse_image_buffer(rj::Document &user_conf) {
  ...
  if (image.HasMember("clear_color")) {
    ...
    if (!(clear_color.IsArray() && clear_color.Size() == 3))
      throw runtime_error("image.clear_color must be an array of 3 real "
...
static Buffer parse_buffer(rj::Value &buffer, string buffer_name,
  ...
  if (buffer_names.count(b.name))
    throw runtime_error("Buffer name " + b.name +
                        " already used (buffers must have unique names)");
  buffer_names.insert(b.name);
  if (!(std::isalpha(b.name[0]) || b.name[0] == '_'))
    throw runtime_error(
        "Invalid buffer name: " + b.name +
        " buffer names must start with either a letter or an underscore");
  ...
  if (buffer.HasMember("clear_color")) {
    ...
    if (!b_clear_color.IsArray() || b_clear_color.Size() != 3) {
      throw runtime_error(b.name + ".clear_color must be an array of 3 real "
                                   "numbers each between 0 and 1");
    }
    for (int i = 0; i < 3; ++i) {
  ...
  if (!buffer.HasMember("size")) {
    cout << "Buffer: " << b.name
         << " does not have a size option, assuming the size of the buffer to "
            "be the window "
            "size"
         << endl;
  } else {
    ...
    if (b_size.IsArray() && b_size.Size() == 2) {
      if (!b_size[0].IsInt() || !b_size[1].IsInt())
        throw runtime_error(b.name +
                            ".size must be an array of two positive integers");
  ...
  if (!buffer.HasMember("geom_iters")) {
    cout << "Buffer: " << b.name
         << " does not have a geom_iters option, assuming the number of times "
            "to execute "
            "geometry shader is 1"
         << endl;
  } else {
...
static vector<Buffer> parse_buffers(rj::Document &user_conf) {
  ...
  for (auto memb = buffers.MemberBegin(); memb != buffers.MemberEnd(); memb++) {
    buffers_vec.push_back(
        parse_buffer(memb->value, memb->name.GetString(), buffer_names));
  }

  return buffers_vec;
}

static vector<int> parse_render_order(rj::Document &user_conf,
  ...
  if (!render_order.IsArray() || render_order.Size() == 0)
    throw runtime_error("render_order must be an array of strings (buffer "
                        "names) with length > 0");
  for (unsigned int i = 0; i < render_order.Size(); ++i) {
    if (!render_order[i].IsString())
      throw runtime_error(
          "render_order can only contain strings (buffer names)");
    ...
    if (index == -1)
      throw runtime_error("render_order member \"" + b_name +
                          "\" must be the name of a buffer in \"buffers\"");
...
static void delete_unused_buffers(vector<Buffer> &buffers,
                                  vector<int> &render_order) {
...
Uniform parse_uniform(rj::Value &uniform, string uniform_name,
                      set<string> &uniform_names) {
  Uniform u;

  if (uniform_names.count(uniform_name))
    throw runtime_error("Uniform name " + uniform_name +
                        " already used (uniforms must have unique names)");
  uniform_names.insert(uniform_name);
  u.name = uniform_name;

  if (uniform.IsArray()) {
    if (uniform.Size() > 4)
      throw runtime_error("Uniform " + u.name +
                          " must have dimension less than or equal to 4");
    ...
    throw runtime_error("Uniform " + u.name +
                        " must be either a number or an array of numbers.");
  }
  return u;
}
...
static vector<Uniform> parse_uniforms(rj::Document &user_conf) {
  ...
  for (auto memb = uniforms.MemberBegin(); memb != uniforms.MemberEnd();
       memb++) {
    uniform_vec.push_back(
        parse_uniform(memb->value, memb->name.GetString(), uniform_names));
  }
...
void ShaderConfig::parse_config_from_string(const std::string &json_str) {
  ...
  rj::ParseResult ok =
      user_conf.Parse<rj::kParseCommentsFlag | rj::kParseTrailingCommasFlag>(
          json_str.c_str());
  if (!ok)
    throw runtime_error(
        "JSON parse error: " + string(rj::GetParseError_En(ok.Code())) +
  ...
  if (user_conf.HasMember("initial_window_size")) {
    ...
    if (!(window_size.IsArray() && window_size.Size() == 2 &&
          window_size[0].IsInt() && window_size[1].IsInt()))
      throw runtime_error(
          "initial_window_size must be an array of 2 positive integers");
  ...
  if (user_conf.HasMember("shader_mode")) {
    if (!user_conf["shader_mode"].IsString())
      throw runtime_error(
          "shader_mode must be either \"easy\" or \"advanced\"");
    if (user_conf["shader_mode"].GetString() == EASY_SHADER_MODE_OPTION) {
      mSimpleMode = true;
    } else if (user_conf["shader_mode"].GetString() ==
               ADVANCED_SHADER_MODE_OPTION) {
      mSimpleMode = false;
    } else {
      throw runtime_error(
          "shader_mode must be either \"easy\" or \"advanced\"");
...
void ShaderConfig::parse_simple_config(const filesys::path &shader_folder) {
  // ignore mBuffers, mRender_order, and mImage options that have been parsed
  // from json
  if (mBuffers.size() != 0) {
    cout << "Warning: ignoring \"buffers\" because shader_mode is set to "
         << EASY_SHADER_MODE_OPTION << endl;
  }
  mBuffers.clear();
  mRender_order.clear();
  mImage = Buffer{};

  // parse file list to decide if frag files should have custom or default
  // geometry shader
...
ShaderConfig::ShaderConfig(const filesys::path &shader_folder,
                           const filesys::path &conf_file_path) {
  ...
  if (filesys::exists(filesys::path(shader_folder / (mImage.name + ".geom")))) {
    ...
    cout << "Warning: Buffer " << mImage.name
         << " is using default geometry shader and has geom_iters > 1 set. "
            "Performance could "
            "suffer."
         << endl;
  }
