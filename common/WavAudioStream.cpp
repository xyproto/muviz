...
struct wav_header_t {
  ...
  unsigned long
      chunkSize;  // 28 [+ sizeof(wExtraFormatBytes) + wExtraFormatBytes] +
                  // sum(sizeof(chunk.id) + sizeof(chunk.size) + chunk.size)
  char format[4]; //"WAVE" = 0x45564157
  char subchunk1ID[4]; //"fmt " = 0x20746D66
  unsigned long
      subchunk1Size; // 16 [+ sizeof(wExtraFormatBytes) + wExtraFormatBytes]
...
WavAudioStream::WavAudioStream(const filesys::path &wav_path) {
  ...
  cout << "Sample Rate * Bits/Sample * Channels / 8: " << header.byteRate <<
  endl; cout << "Bits per Sample * Channels / 8.1: " << header.blockAlign <<
  endl; cout << "Bits per Sample: " << header.bitsPerSample << endl;
  */
...
int WavAudioStream::get_sample_rate() { return sample_rate; }

int WavAudioStream::get_max_buff_size() { return max_buff_size; }
