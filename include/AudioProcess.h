...
// TODO ifdef a scalar on the fft frequency because fft amplitude is much
// smaller on linux than on windows
...
// length of visualizer 1D texture buffers. Number of frames of audio this
// module outputs.
static const int VL = VISUALIZER_BUFSIZE;

// I think the FFT looks best when it has 8192/48000, or 4096/24000, time
// granularity. However, I like the wave to be 96000hz just because then it fits
// on the screen nice. To have both an FFT on 24000hz data and to display
// 96000hz waveform data, I ask PipeWire to output 48000hz and then resample
// accordingly. I've compared the 4096 sample FFT over 24000hz data to the 8192
// sample FFT over 48000hz data and they are surprisingly similar. I couldn't
// tell a difference really. I've also compared resampling with ffmpeg to my
// current naive impl. Hard to notice a difference.

#ifdef HAVE_PAR_ALGS
// Cross correlation sync options
// store the n most recent buffers sent to the visualizer (stores contents of
// visualizer buffer).
static const int HISTORY_NUM_FRAMES = 50;
// search an interval of samples centered around the current read index (r) for
...
// HISTORY_SEARCH_RANGE * HISTORY_NUM_FRAMES * HISTORY_BUFF_SIZE /
// HISTORY_SEARCH_GRANULARITY

// AudioProcess does not create AudioStream because ProceduralAudioStream must
// be created by AudioProcess's owner
...
private:
  ...
  // The audio pointer in the circular buffer should be moved such that it
  // travels only distances that are whole multiples, not fractional multiples,
  // of the audio wave's wavelength. But how do we find the wavelength of the
  // audio wave? If we try and look for the zero's of the audio wave and note
  // the distance between them, then we will not get an accurate or consistent
  // measure of the wavelength because the noise in the waveform will likely
  // throw our measure off.
  //
  ...
  // Increases similarity between successive frames of audio output by the
  // AudioProcess
  bool xcorr_sync;
  ...
  // Returns the bin holding the max frequency of the fft. we only consider the
  // first 100 bins.
  ...
  // If w is within VL units in front of r, then adjust r so that this is no
  // longer true. w: index of the writer in the circular buffer r: index of the
  // reader in the circular buffer freq: frequency to compute the wavelength
  // with
  static int advance_index(int w, int r, float freq, int tbl);

  // Compute the dot product between a(t + a_offset) and b(b_size-t)
  static float reverse_dot_prod(const float *a, const float *b, int a_offset,
  ...
  static int cross_correlation_sync(const int w, const int r, const int dist,
                                    ...
                                    const int frame_id, const float *buff);
...
AudioProcess<ClockT, AudioStreamT>::AudioProcess(AudioStreamT &_audio_stream,
  ...
  if (audio_stream.get_sample_rate() != 48000) {
    std::cout << "The AudioProcess is meant to consume 48000hz audio but the "
                 "given AudioStream "
              << "produces " << audio_stream.get_sample_rate() << "hz audio."
              << std::endl;
    exit(1);
  }
  if (audio_stream.get_max_buff_size() < ABL) {
    std::cout << "AudioProcess needs at least " << ABL
              << " frames per call to get_next_pcm"
  ...
  fft_out_l =
      (complex<float> *)Aligned_Alloc(32, (N / 2 + 1) * sizeof(complex<float>));
  fft_out_r =
      (complex<float> *)Aligned_Alloc(32, (N / 2 + 1) * sizeof(complex<float>));
...
void AudioProcess<ClockT, AudioStreamT>::step() {
  ...
  if (now_time > next_time) {
    ...
    if (xcorr_sync) {
      ...
      reader_l = cross_correlation_sync(writer, reader_l, HISTORY_SEARCH_RANGE,
                                        history_buff_l, frame_id, audio_buff_l);
      reader_r = cross_correlation_sync(writer, reader_r, HISTORY_SEARCH_RANGE,
                                        history_buff_r, frame_id, audio_buff_r);
    }
    ...
    // Rescale with a delay so the rescaling is less obvious and unnatural
    // looking
    ...
    for (int i = 0; i < VL; ++i) {
      float sample_l =
          .66f * audio_buff_l[(i + reader_l) % TBL] / (channel_max_l + 0.0001f);
      float sample_r =
          .66f * audio_buff_r[(i + reader_r) % TBL] / (channel_max_r + 0.0001f);

      audio_sink.audio_l[i] =
          mix(audio_sink.audio_l[i], sample_l, wave_smoother);
      audio_sink.audio_r[i] =
          mix(audio_sink.audio_r[i], sample_r, wave_smoother);

      audio_sink.freq_l[i] =
          mix(audio_sink.freq_l[i],
              std::abs(fft_out_l[i]) / std::sqrt(float(FFTLEN)), fft_smoother);
      audio_sink.freq_r[i] =
          mix(audio_sink.freq_r[i],
...
int AudioProcess<ClockT, AudioStreamT>::max_bin(const complex<float> *f) {
  float max_norm = 0.f;
  int max_i = 0;
  // catch frequencies from 5.86 to 586 (that is i * SRF / FFTLEN for i from 1
  // to 100)
  for (int i = 1; i < 100; ++i) {
...
template <typename ClockT, typename AudioStreamT>
int AudioProcess<ClockT, AudioStreamT>::dist_forward(int from, int to,
                                                     int tbl) {
  int d = to - from;
  if (d < 0)
    d += tbl;
...
template <typename ClockT, typename AudioStreamT>
int AudioProcess<ClockT, AudioStreamT>::dist_backward(int from, int to,
                                                      int tbl) {
  return dist_forward(to, from, tbl);
}

template <typename ClockT, typename AudioStreamT>
float AudioProcess<ClockT, AudioStreamT>::max_frequency(
...
template <typename ClockT, typename AudioStreamT>
float AudioProcess<ClockT, AudioStreamT>::get_harmonic_less_than(float freq,
                                                                 float thres) {
...
template <typename ClockT, typename AudioStreamT>
int AudioProcess<ClockT, AudioStreamT>::advance_index(int w, int r, float freq,
                                                      int tbl) {
  ...
  //    cout << "reader too close: " << df << "\twave_len: " << wave_len <<
  //    endl;
...
int AudioProcess<ClockT, AudioStreamT>::cross_correlation_sync(
    const int w, const int r, const int dist,
    float *history_buff[HISTORY_NUM_FRAMES], const int frame_id,
    const float *buff) {
  ...
  std::for_each(indices.begin(), indices.end(),
                ...
                [&](const int i) {
                  const int local_r =
                      (r_begin + i * HISTORY_SEARCH_GRANULARITY) % TBL;
                  float dot = 0.f;
                  for (int b = 0; b < HISTORY_NUM_FRAMES; ++b) {
                    int cur_buf = (frame_id + b) % HISTORY_NUM_FRAMES;
                    dot += reverse_dot_prod(buff, history_buff[cur_buf],
                                            local_r, TBL, HISTORY_BUFF_SIZE);
                  }
                  dots[i] = {local_r, dot};
                });

  std::sort(dots.begin(), dots.end(),
            [](const result &x, const result &y) { return x.dot > y.dot; });
  return dots[0].r;
}

template <typename ClockT, typename AudioStreamT>
typename ClockT::duration AudioProcess<ClockT, AudioStreamT>::dura(float x) {
  // dura() converts a second, represented as a double, into the appropriate
  // unit of time for ClockT and with the appropriate arithematic type using
  // dura() avoids errors like this : chrono::seconds(double initializer) dura()
  // : <double,seconds> -> chrono::steady_clock::<typeof count(), time unit>
