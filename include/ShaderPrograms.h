...
public:
  ShaderPrograms(const ShaderConfig &config, const Renderer &renderer,
                 const Window &window, const filesys::path &shader_folder);
...
private:
  ...
  void compile_buffer_shaders(const filesys::path &shader_folder,
                              const std::string &buff_name,
                              const std::string &uniform_header,
                              const bool uses_default_geometry_shader);
