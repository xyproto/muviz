...
public:
  FileWatcher(filesys::path shader_folder)
      : shader_folder(shader_folder), shaders_changed(false),
        last_event_time() {
    file_watcher.addWatch(shader_folder.string(), (FW::FileWatchListener *)this,
                          false);
  }

  ~FileWatcher() { file_watcher.removeWatch(shader_folder.string()); }

  // More than one event can be delivered by the editor from a single save
  // command. So sleep a few millis and then process the event and set the last
  // process time. If new event is within 100 ms of last process time, then
  // ignore it. If shader.json or any frag or geom file has changed, then set
  // shaders_changed.
  void handleFileAction(FW::WatchID watchid, const FW::String &dir,
    ...
    if (extension != ".json" && extension != ".geom" && extension != ".frag")
      return;
    if (std::chrono::steady_clock::now() - last_event_time <
